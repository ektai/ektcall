...
const validateRoleMapping = (mappedRoles) => {
	...
	if (!validRolesMapping) {
		throw new Error('Please verify your mapping for LDAP X EktAi  Roles. There is some invalid Rocket Chat Role.');
	}
};
...
const validateLDAPRolesMappingStructure = (mappedRoles) => {
	...
	if (!validStructureMapping) {
		throw new Error('Please verify your mapping for LDAP X EktAi  Roles. The structure is invalid, the structure should be an object like: {key: LdapRole, value: [An array of rocket.chat roles]}');
	}
};
