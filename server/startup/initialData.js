	...
	Meteor.defer(() => {
		...
		if (process.env.TEST_MODE === 'true') {
			...
			const adminUser = {
				...
				name: 'EktAi  Internal Admin Test',
			...
			if (Users.findOneByEmailAddress(adminUser.emails[0].address)) {
				throw new Meteor.Error(`Email ${ adminUser.emails[0].address } already exists`, 'Ekt.Ai can\'t run in test mode');
			}

			if (!checkUsernameAvailability(adminUser.username)) {
				throw new Meteor.Error(`Username ${ adminUser.username } already exists`, 'Ekt.Ai can\'t run in test mode');
			}
