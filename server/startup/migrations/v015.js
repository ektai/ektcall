		...
		oldFilesCollection.find({
			...
			Messages.find({
				$or: [{
					...
					'urls.url': `https://ekt.ai/cfs/files/Files/${ cfsRecord._id }`,
				}],
			}).forEach((message) => {
				for (const urlsItem of message.urls) {
					if (urlsItem.url === `https://open.rocket.chat/cfs/files/Files/${ cfsRecord._id }` || urlsItem.url === `https://ekt.ai/cfs/files/Files/${ cfsRecord._id }`) {
						...
						message.msg = message.msg.replace(`https://ekt.ai/cfs/files/Files/${ cfsRecord._id }`, Meteor.absoluteUrl() + url);
					}
				}
